...
BOOL ExitTask(pthread_t pTaskId, const char *name)
{
	if((pTaskId != list.tId) && (strcmp(name,list.tName) != 0))
	{
		fprintf(stderr,"ERROR : ExitTask Failed : UnKnown Task (%s) \n",name);
		return FALSE;
	}
	pthread_exit((void *)0);
	return TRUE;
